...
const PrintAccessPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.message) {
        ...
        setMessage('An unexpected response was received. Please check the console for details.');
      }
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE_URL}/api/jobs/reprint/${jobToken}`, {
        connectorId: connectorId,
        printerName: printerName,
        fileSecret: secret // Re-send secret for verification if needed
      });

      setMessage(`Print job sent to ${printerName} on ${connectorName}.`); // Updated message
      setError('');
      setShowPrinterSelection(false);
    } catch (err) {
      if (axios.isAxiosError(err) && err.response) {
      ...
      <div className="form-card" style={{ maxWidth: '450px', margin: 'auto', padding: '2.5rem' }}>
        ...
        {!fileContentUrl && !showPrinterSelection && !isPollingForPrinters && ( // Only show form if we don't have file URL or printer selection yet, and not polling
          <form onSubmit={handleSubmit}>
            ...
            <button type="submit" className="button-link" style={{ backgroundColor: 'var(--accent-color)', color: 'var(--dark-background)', fontSize: 'var(--button-font-size)', padding: 'var(--button-padding)' }} disabled={isAccessingJob}>
        ...
        {showPrinterSelection && availableConnectors.length > 0 && ( // Show printer selection if enabled and printers are available
          <div style={{ marginTop: '2rem' }}>
            ...
            <button onClick={handlePrint} className="button-link" style={{ backgroundColor: 'var(--success-color)', color: 'var(--dark-background)', fontSize: 'var(--button-font-size)', padding: 'var(--button-padding)' }} disabled={isPrinting || !selectedPrinter || selectedPrinter === ""}>
        ...
        {workerDownloadLink && !isPollingForPrinters && ( // Display download link only if worker not detected AND not polling
          <div style={{ marginTop: '1.5rem' }}>
            <p style={{ color: 'var(--light-text-color)' }}>The print connector service is not detected on your PC. You can download it here:</p>
            <a href={workerDownloadLink} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--accent-color)' }}>Download Print Connector</a>
          </div>
        )}
        {fileContentUrl && (
          <div style={{ marginTop: '2rem', width: '100%', height: 'calc(100vh - 250px)' }}>
            <p style={{ color: 'var(--light-text-color)', marginBottom: '1rem' }}>Your file is ready for viewing or downloading:</p>
            <a href={fileContentUrl} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--primary-color)', marginBottom: '1rem', display: 'inline-block' }}>Download File</a>
